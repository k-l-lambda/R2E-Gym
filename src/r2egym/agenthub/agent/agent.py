...
# MAX_CONTEXT_TOKENS: upper bound for the litellm.token_counter() pre-check inside
# model_query() (see _count_tokens below). When exceeded, a ValueError is raised and
# the instance exits with exit_reason='llm_query_error'.
#
# BUG HISTORY (discovered 2026-03-04, eval of DeepSWE-Preview on youyun.38):
#   Original value was 65536, later bumped to 98304 in commit 4831630 -- both too low.
#   Root cause: _count_tokens() calls litellm.token_counter(), which falls back to
#   tiktoken (OpenAI cl100k_base BPE, ~100K vocab) for any non-GPT model name.
#   Qwen3's BPE has ~150K vocab and aggressively merges tokens, especially for Chinese
#   reasoning text inside <think>...</think> blocks. tiktoken splits these into far more
#   sub-tokens, overcounting by ~6x versus the real Qwen3 token count.
#   Consequence: a conversation using ~11K real Qwen3 tokens was reported as 65K+ by
#   tiktoken, triggering a false pre-check failure. At 65536 threshold, ~82% of
#   DeepSWE-Preview instances were killed prematurely; at 98304, still ~28% failed.
#
# FIX: Set to 1_000_000 to effectively disable this inaccurate pre-check.
#   The real 64K context limit is correctly enforced by the post-response check in the
#   agent step loop (~line 522 of this file):
#       elif total_tokens >= max_token_limit:   # uses usage.total_tokens from vLLM API
#   That value comes directly from the API response and reflects the true Qwen3 count.
#
# TODO: Either remove _count_tokens() pre-check entirely (vLLM already enforces the
#   hard limit), or replace litellm.token_counter() with the model's actual tokenizer.
...
class Agent:
    ...
    def _count_tokens(self, messages: List[Dict[str, str]]) -> int:
        ...
        WARNING: litellm.token_counter() falls back to tiktoken (OpenAI BPE) for any
        model name that is not a known OpenAI/Anthropic model. tiktoken overcounts
        tokens for Qwen3-based models (e.g., DeepSWE-Preview) by ~6x because their
        BPE vocabularies differ significantly. This method is used only for the
        MAX_CONTEXT_TOKENS pre-check, which is currently set to 1_000_000 to disable
        the check. For accurate token counts, use usage.total_tokens from the API
        response instead (see agent step loop for the authoritative check).
        """
